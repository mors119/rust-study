...
- 어떤 값을 "가리키는 타입"이지만, 단순 주소 저장을 넘어서 추가 동작까지 함께 가진 타입입니다.
스마트 포인터(Smart Pointer)는 메모리의 주소를 나타내는 포인터와 같은 역할 및 부가적인 기능을 가진 데이터 구조를 얘기한다.

Rust에서 변수 앞에 &를 붙이면 해당 변수값이 들어 있는 메모리 주소를 가리키는 포인터가 된다.
반면에 스마트 포인터라 할 수 있는 Vec 나 Box 같은 것은 그 객체를 생성하면 객체 내에 힙 메모리의 위치를 가리키는 포인터가 있어서, 마치 포인터 처럼 사용할 수 있다.
이러한 객체 들을 '스마트 포인터'라고 한다.
